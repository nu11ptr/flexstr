    ...
    FlexCStr<'str, SIZE, BPAD, HPAD, HEAP>
{
    ...
    ///
    /// let s = LocalCStr::from_static(
    ///     CStr::from_bytes_with_nul(b"This is a string literal\0").unwrap(),
    /// );
    /// assert!(s.is_static());
    /// ```
    #[inline(always)]
    ...
    HEAP: Storage<CStr>,
{
    ...
    /// Force the creation of a heap allocated string. Unlike to/into/[from_ref](FlexCStr::from_ref)
    /// functions, this will not attempt to inline first even if the string is a candidate for inlining.
    ///
    /// # Note
    /// Using this is only recommended when using the associated [to_heap](FlexCStr::to_heap)
    /// and [try_to_heap](FlexCStr::try_to_heap) functions.
    /// ```
    /// use std::ffi::CStr;
    /// use flexstr::FlexStrCore;
    /// use flexstr::c_str::LocalCStr;
    ///
    /// let s = LocalCStr::from_ref_heap(
    ///     CStr::from_bytes_with_nul(b"This is too long to inline!\0").unwrap(),
    /// );
    /// assert!(s.is_heap());
    /// ```
    #[inline(always)]
    pub fn from_ref_heap(s: impl AsRef<CStr>) -> Self {
    ...
    /// use flexstr::c_str::LocalCStr;
    ///
    /// let s = LocalCStr::from_borrow(
    ///     CStr::from_bytes_with_nul(b"This is a string literal\0").unwrap(),
    /// );
    /// assert!(s.is_borrow());
    /// ```
    #[inline(always)]
    pub fn from_borrow(s: &'str CStr) -> Self {
